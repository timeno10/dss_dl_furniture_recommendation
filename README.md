...
### Process
1. 사전 학습 모델을 활용하여 객체 탐지 및 추출 (Object Detection and Extraction)
2. 특징 추출 (Feature Extraction)
3. 유사도 측정 (Similarity Measure) 후 추천 시스템 (Recommendation System) 구축
4. 6가지 카테고리로 직접 라벨링한 custom 데이터셋으로 모델링
5. 다양한 모델 성능 비교 후 최적의 모델 선정
...
- Annoy 라이브러리를 활용하여 유사도(최근접 이웃) 측정
